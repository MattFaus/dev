...
class MtimeFileWatcher(object):
  ...
  def _generate_filename_to_mtime(self):
    ...
    for dirname, dirnames, filenames in os.walk(self._directory,
                                                followlinks=True):
      reldir = os.path.relpath(dirname, self._directory)
      
      # HACK(mattfaus): To get around file limitation
      if (reldir.startswith('.git') or
          reldir.startswith('clienttemplates/') or
          reldir.startswith('genfiles/') or
          reldir.startswith('deploy/') or
          reldir.startswith('datastore/')):
        continue
